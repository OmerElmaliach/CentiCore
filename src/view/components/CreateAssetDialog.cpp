...
void CreateAssetDialog::accept() {
    ...
    bool isNumber;
    QString symbol = m_ui->symbolInput->text(), shares = m_ui->sharesInput->text();
    shares.toDouble(&isNumber);
    ...
    bool wasAdded = AssetsController::getInstance().add(symbol.toStdString(), shares.toDouble(), 0, m_type);

    if (wasAdded) {
        emit assetCreated(symbol, shares, m_type); 
    }
